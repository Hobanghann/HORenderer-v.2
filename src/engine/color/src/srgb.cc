...
SRGB::SRGB(const LinearRGB& lrgb)
    : red_(MathUtils::Pow(lrgb.red(), kINVERSE_GAMMA)),
      green_(MathUtils::Pow(lrgb.green(), kINVERSE_GAMMA)),
      blue_(MathUtils::Pow(lrgb.blue(), kINVERSE_GAMMA)),
      alpha_(lrgb.alpha()) {}
...
std::uint32_t SRGB::ToARGB32() const {
  std::uint32_t a_bit =
